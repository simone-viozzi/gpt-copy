        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with runner.isolated_filesystem(temp_dir=temp_dir):
                ...
                result = runner.invoke(
                    main, ["test", "-e", "test/dropbox/*", "--tree-only"]
                )
    ...
    def test_exclusion_pattern_directory_itself(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with runner.isolated_filesystem(temp_dir=temp_dir):
                ...
                # dropbox directory should still appear (compression shows limited content)
                assert "dropbox" in result.output
                # The other file should still be included
                assert "should_be_included.txt" in result.output
                # The compression behavior is working correctly if we see the specific connector pattern
