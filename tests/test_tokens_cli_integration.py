...
def test_tokens_with_top_n():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        (temp_path / "large.py").write_text("# This is a long comment\nprint('hello world')\n# Another comment")
        
        # Run gpt-copy with --tokens --top-n 2
        returncode, stdout, stderr = run_gpt_copy_command([str(temp_path), "--tokens", "--top-n", "2"])
...
def test_regular_functionality_still_works():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        returncode, stdout, stderr = run_gpt_copy_command([str(temp_path), "--tree-only"])
...
def test_tokens_without_top_n():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        returncode, stdout, stderr = run_gpt_copy_command([str(temp_path), "--top-n", "1", "--tree-only"])
